...
def create_python_dataset():
    with open('python_while_dataset.py', 'w') as f:
        for i in range(1, 1000):

            random_line_one = random.choice(
                list(open('python_assignment_dataset.py')))
...
def create_java_dataset():
    ...
    with open('java_while_dataset.java', 'w') as f:
        for i in range(1, 1000):
            random_line_one = random.choice(
                list(open('java_assignment_dataset.java')))
            random_line_two = random.choice(
                list(open('java_declaration_dataset.java')))
            random_line_three = random.choice(
                list(open('java_assignment_dataset.java')))
...
def create_cpp_dataset():
    with open('cpp_while_dataset.cpp', 'w') as f:
        for i in range(1, 1000):
            random_line_one = random.choice(
                list(open('cpp_assignment_dataset.cpp')))
            random_line_two = random.choice(
                list(open('cpp_declaration_dataset.cpp')))
            random_line_three = random.choice(
                list(open('cpp_assignment_dataset.cpp')))
