...
class Concept:
    ...
    def __init__(self):
        ...
        if not os.path.isfile("keywords_concept.txt"):
            ...
            with open("keywords_concept.txt", encoding="utf8") as file:
                ...
                print(
                    f"Loading {len(self.tree_concepts)} keywords for concepts ... Done ...")
    ...
    def extend_concept(self, cpp, java, python, key):
        ...
        if generic_cpp and len(generic_cpp) - len(generic_cpp.lstrip()) == 0 and generic_jv and generic_py:
            ...
            if not flag:
                self.concepts[key].append(
                    [generic_cpp, generic_jv, generic_py])
    ...
    def add_concept(self, cpp, java, python, key):
        ...
        if generic_cpp and generic_jv and generic_py:
            self.concepts.update(
                {key: [[generic_cpp, generic_jv, generic_py]]})
        ...
        if input_language == JAVA:
            ...
            for concept in self.tree_concepts:
                if lang_id in concept:
                    tree = parser_jv.parse(
                        bytes(self.concepts[concept][0][0], "utf-8")).root_node.sexp()
                    lang_matches.append(tree)
                    final_concept = process.extractOne(
                        parse_tree, lang_matches, scorer=fuzz.partial_ratio)
        ...
        if input_language == PYTHON:
            ...
            for concept in self.tree_concepts:
                if lang_id in concept:
                    ...
                    if final_concept[-1] >= 80:
                        concept_final = concept
        print(concept_final + "has thet highest posssibility of being a correct match. Suggestion for optimized code is in suggestion.txt")
